...
from custom_ultils import get_schedule
...
def handle_date_time(user_content):
    """Return the current date and time information."""
    try:
        current_time = time.time()
        local_time = time.localtime(current_time)
        
        # Format the date and time in multiple useful formats
        formatted_data = {
            "ISO format": time.strftime("%Y-%m-%d %H:%M:%S", local_time),
            "Date": time.strftime("%A, %B %d, %Y", local_time),
            "Time": time.strftime("%I:%M:%S %p %Z", local_time),
            "Unix timestamp": str(int(current_time))
        }
...
def handle_schedule_command(user_content):
    """Parse a schedule command and return schedule information."""
    match = re.search(r'@lich\s+(\d+)', user_content)
    if not match:
        return "Please provide a valid student ID with the format '@lich 123456789'"
    
    student_id = match.group(1)
    date = datetime.now().strftime("%d%m%Y")
    

    schedule_data = get_schedule(date, student_id)
    
    return f"Today schedule for student with ID {student_id}:\n{schedule_data}"
...
def chat():
    ...
    if isinstance(user_content, str):
        ...
        elif user_content.lower().startswith("@time") or user_content.lower().startswith("@date"):
            additional_text = handle_date_time(user_content)
            user_content = "Current date and time information:"
            system_content = "You are an assistant providing accurate date and time information."
        
        elif user_content.lower().startswith("@lich"):
            additional_text = handle_schedule_command(user_content)
            user_content = "Schedule for today"
            system_content = "You are an assistant providing schedule information"
